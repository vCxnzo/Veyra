...
import { Search, Bell, Plus, ShieldCheck, Gamepad2, ChevronRight, User } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchQuery.trim()) {
      onToast('Global Search Executed', `Searching records for "${searchQuery}" in ${activeCommunity.name}`);
    }
  };
  ...
  return (
    <header className="h-16 bg-slate-950/90 border-b border-slate-800/80 px-6 flex items-center justify-between sticky top-0 z-20 backdrop-blur-md">
      ...
      <div className="flex items-center gap-2">
        <span className="text-xs text-slate-400 font-medium">{activeCommunity.name}</span>
        <ChevronRight className="w-3.5 h-3.5 text-slate-600" />
        <h1 className="text-sm font-bold text-white tracking-tight">{tabTitles[activeTab]}</h1>
      </div>

      {/* Center Search Bar */}
      <form onSubmit={handleSearchSubmit} className="hidden md:flex items-center w-72 relative">
      ...
      <div className="flex items-center gap-3">
        {activeTab === 'shifts' && onOpenCreateShift && (
          ...
          >
            <Plus className="w-3.5 h-3.5" /> Host Shift
          </button>
        )}
          ...
          {notificationsOpen && (
            <div className="absolute right-0 mt-2 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-4 z-50 space-y-3">
              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                <h4 className="text-xs font-bold text-white">Community Notifications</h4>
                <span className="text-[10px] text-indigo-400 font-mono">3 New</span>
              </div>
              <div className="space-y-2 text-xs">
                <div className="p-2 rounded-lg bg-slate-950 border border-slate-800">
                  <p className="font-semibold text-white">Shift Completed</p>
                  <p className="text-slate-400 text-[11px]">Shift #101 hosted by Matthew finished with 14 participants.</p>
                </div>
                <div className="p-2 rounded-lg bg-slate-950 border border-slate-800">
                  <p className="font-semibold text-white">New HR Application</p>
                  <p className="text-slate-400 text-[11px]">Brandon_Dev submitted a Supervisor application.</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex items-center gap-2.5 pl-2 border-l border-slate-800">
          ...
          <div className="hidden lg:block text-left">
            <span className="text-xs font-semibold text-white block leading-none">Matthew</span>
            <span className="text-[10px] text-slate-400 block mt-0.5">General Manager</span>
          </div>
        </div>
      </div>
    </header>
